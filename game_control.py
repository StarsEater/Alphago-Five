...
class board(object):
    ...
    def move_id2move_action(self):
        return [(x, y) for x in range(self.border_x) for y in range(self.border_y)]
    ...
    def _valid_move_action(self, move_action):
        x, y = move_action
        return self.state_board[x][y] == args.placeholder_id and 0 <= x < self.border_x and 0 <= y < self.border_y

    def next_valid_move_action(self):
        next_move_actions = list(filter(
            lambda act: self._valid_move_action(act),
            [(i, j) for i in range(self.border_x)
             for j in range(self.border_y)]
        ))
        next_move_ids = [self.move_action2move_id[act]
                         for act in next_move_actions]
...
class game(object):
    ...
    def __init__(self, board):
        ...
        self.player_id2player_color = {
            ...
            args.second_player_id: '白' if args.first_player_color == '黑'
            else '黑',
            args.placeholder_id: '空'
        }
        self.player_color2player_id = {
            v: k for k, v in self.player_id2player_color.items()}
    ...
    def take_move_action(self, player):
        ...
        # 玩家决策
        action_id = player.get_action(self.state_board, next_move_action_id)
    ...
    def state_update(self, cur_move_action):
        ...
        self.last_move_id = self.board.move_action2move_id[self.last_move_action]
    ...
    def flip_move_action(self, move_action, center_pos):
        ...
        pos_symm = [rotate_90, rotate_180, rotate_270,
                    horiz_symm, vert_symm, diag_symm, bdiag_symm]
        return pos_symm
    ...
    def encode_state2numpy(self):
        ...
        for k in range(0, last_k//2):
            ...
            state_board_np[k+last_k//2] = (self.board_state_deque[k*2+1]
                                           [:, :] == self.rival_player_id)

        x, y = self.last_move_action
        state_board_np[last_k][x][y] = 1
        for i in range(self.border_x):
            for j in range(self.border_y):
                state_board_np[last_k +
                               1][i][j] = int(self.state_board[i][j] == args.placeholder_id)
